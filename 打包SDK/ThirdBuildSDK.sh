...
#  XappRun
#
#  Created by ihooyah on 2019/10/12.
#  Copyright © 2019 moyun. All rights reserved.
...
#build之前clean一下
xcodebuild clean -workspace "${target_Name}.xcworkspace" -scheme "${target_Name}" -configuration Release -sdk iphoneos
xcodebuild clean -workspace "${target_Name}.xcworkspace" -scheme "${target_Name}" -configuration Release -sdk iphonesimulator
#
#
...
#模拟器build
xcodebuild build -workspace "${target_Name}.xcworkspace" -scheme "${target_Name}" -configuration Release -sdk iphoneos SYMROOT="$build_DIR"
xcodebuild build -workspace "${target_Name}.xcworkspace" -scheme "${target_Name}" -configuration Release -sdk iphonesimulator SYMROOT="$build_DIR"
...
#移除i386
lipo "${INSTALL_DIR_A}" -remove i386 -output "${INSTALL_DIR_A}"
echo "======合成结束======"
